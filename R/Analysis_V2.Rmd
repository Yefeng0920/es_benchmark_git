...
# prepare data
dat_long <- data.frame(Approach = c(rep("Individual level", nrow(dat_eco)), rep("Meta-analytic", nrow(df_eco))),
...
```{r equivalence}
library("TOSTER")
TOSTmeta(ES = 0.3, se = 0.12, low_eqbound_d = - quantile(df_psy$folded_mu_adj, probs = c(.25, .5, .75))[1], high_eqbound_d = quantile(df_psy$folded_mu_adj, probs = c(.25, .5, .75))[1], alpha = 0.05)
...
dgof::ks.test(dat_eco$mu_obs, df_eco$mu_unadj)
...
# Wilcoxon signed-rank test to compare the central tendency if the data is paired.
wilcox_test <- wilcox.test(dat_eco$mu_obs, df$adjusted_mu, paired = TRUE)

# Perform Mann-Whitney U Test (Wilcoxon signed-rank test) This non-parametric test compares the medians of two independent groups
wilcox_eco <- wilcox.test(dat_eco$mu_obs, df_eco$mu_unadj)


#F-test to compare the variances of two groups. For non-parametric comparison of variances, you can use the Levene's test or Fligner-Killeen test.
...
levene_eco <- leveneTest(Estimate ~ Approach, data = dat_long)
print(levene_eco)

# Perform Fligner-Killeen test to compare variances
fligner_test <- fligner.test(Estimate ~ Approach, data = dat_long)
...
boot_skewness_unadjusted <- boot(df$unadjusted_mu, skewness_func, R = 1000)
boot_skewness_adjusted <- boot(df$adjusted_mu, skewness_func, R = 1000)

# Bootstrapping for kurtosis
boot_kurtosis_unadjusted <- boot(df$unadjusted_mu, kurtosis_func, R = 1000)
boot_kurtosis_adjusted <- boot(df$adjusted_mu, kurtosis_func, R = 1000)

# Confidence intervals for skewness
ci_skewness_unadjusted <- boot.ci(boot_skewness_unadjusted, type = "bca")
ci_skewness_adjusted <- boot.ci(boot_skewness_adjusted, type = "bca")

# Confidence intervals for kurtosis
ci_kurtosis_unadjusted <- boot.ci(boot_kurtosis_unadjusted, type = "bca")
ci_kurtosis_adjusted <- boot.ci(boot_kurtosis_adjusted, type = "bca")
